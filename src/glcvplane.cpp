...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneCreate (float w, float h)
{
	int i = 0;
	int j = 0;
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneTranslate (float x, float z, float y)
{
 	float trans[16] = {0};
	float aux[16] = {0};
	int i;
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneMove	(int axis, float value)
{
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneRotate	(int axis, float angle)
{
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneScale (int axis, float value)
{
	float aux[16] = {0};
	float scale[16] = {0};
	int i;
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneLoadIdentity (void)
{
	LoadIdentityMatrix4x4 (_modelview_matrix);	
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneReset (void)
{
	_xangle = 0;
	_yangle = 0;
	_zangle = 0;
...
// Desc: Texturize a determined cube face with the given texture
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneSetTexBin (IplImage *texture_data)
{
	cvCvtColor (texture_data, _texture_data, CV_GRAY2RGB);
	glBindTexture(GL_TEXTURE_2D, _texture);
	glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data->imageData);
	//glTexDirectVIV (GL_TEXTURE_2D, _texture_w, _texture_h, GL_VIV_YV12, (void **)_texture_data->imageData);
}
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneSetTexBuf (char *texture_data, int w, int h)
{
	glBindTexture(GL_TEXTURE_2D, _texture);
	//glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, texture_data);
	glTexImage2D (GL_TEXTURE_2D, 0,	GL_RGB,	w, h, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, texture_data);

}

//--------------------------------------------------------------------------------------
// Name: PlaneSetFaceColor()
// Desc: Paint a determined face with the giver color
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneSetColor (CvScalar color)
{
	cvSet (_texture_data, color, NULL);
	glBindTexture(GL_TEXTURE_2D, _texture);
	glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data->imageData);
...
//--------------------------------------------------------------------------------------
float GLCVPlane::PlaneGetAxisAngle (int axis)
{
	float angle = 0;
...
//--------------------------------------------------------------------------------------
float GLCVPlane::PlaneGetAxisPos (int axis)
{
	float pos = 0;
...
void GLCVPlane::PlaneShowCoords (void)
{
	int i = 0;
	int j = 0;
	int k = 0;
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneShowUVCoords (void)
{
	int i = 0;
	int j = 0;
	int k = 0;
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneShowModelMatrix (void)
{
	int i;
	int j = 0;
...
//--------------------------------------------------------------------------------------
void GLCVPlane::MultMatrix4x4 (float *matC, float *matA, float *matB)
{
...
//--------------------------------------------------------------------------------------
void GLCVPlane::LoadIdentityMatrix4x4 (float *m)
{
	m[0] = 1;
	m[1] = 0;
	m[2] = 0;
...
//--------------------------------------------------------------------------------------
GLuint GLCVPlane::GenTextures (void)
{
	GLuint texture_id = 0;
...
void GLCVPlane::PlaneCreateTex	(int texture_w, int texture_h, int ch)
{
	...
	_texture_data	= cvCreateImage (cvSize (_texture_w, _texture_h), 8, ch);
...
//--------------------------------------------------------------------------------------
// Name: PlaneDestroyTex()
// Desc: dealocates cube textures
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneDestroyTex (void)
{
	cvReleaseImage (&_texture_data);
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneSetProjMatrix (float *projection_matrix)
{
...
//--------------------------------------------------------------------------------------
bool GLCVPlane::CreateShaderProgram (void)
{
...
//--------------------------------------------------------------------------------------
void GLCVPlane::PlaneDestroy (void)
{
	// Frees the texture
...
void GLCVPlane::PlaneLoadGLTextures (EGLDisplay egldisplay, IplImage *texture)
{
	/*//Setup eglImage
	...
	*/
	//16 FPS
	/*
	for (int i = 0; i < texture->imageSize; i+=4) 
	{
		imageBuffer[i] = texture->imageData[i];
		imageBuffer[i+1] = texture->imageData[i+1];
		imageBuffer[i+2] = texture->imageData[i+2];
		imageBuffer[i+3] = texture->imageData[i+3];
	}
	*/
	...
	for (int i = 0; i < texture->height; i++) 
	{
		...
		{
			ptr = cvPtr2D (texture, i, j, 0);
			
			imageBuffer[sum]   = ptr[0];
			imageBuffer[sum+1] = ptr[1];
			imageBuffer[sum+2] = ptr[2];
			imageBuffer[sum+3] = ptr[3];
